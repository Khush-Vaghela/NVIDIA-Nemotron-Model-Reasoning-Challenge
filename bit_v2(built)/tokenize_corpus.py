"""Tokenize bit_v2 into the exact corpus segment format, enforcing conciseness caps.

Identical mechanics to numeral_v2: masked prompt (chat template) + trained
completion `{reasoning}\n</think>\n\boxed{answer}<|im_end|>`, interleaved
segments, byte-exact round-trip with two independent decoders per entry.

HARD conciseness enforcement on COMPLETION (trained) tokens:
  every trace <= 4500; distribution must satisfy median < 3500 and p90 < 4000
...
def main() -> None:
    ...
    for pid, p in problems.items():
        ...
        assert boxed[-1] == p["answer"] and re.fullmatch(r"[01]{8}", boxed[-1]), pid
    ...
    print(f"(original corpus bit traces: median ~6,735, p90 ~7,200 — the thing we are fixing)")
    ...
    print("All entries passed byte-exact round-trip + boxed/binary-format checks.")
