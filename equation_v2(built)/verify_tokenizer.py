"""MANDATORY GATE for equation_v2: prove tokenizer fidelity before any generation.

Same 4 checks as bit_v2, referenced against 3 EXISTING equation_numeric_deduce entries
from the attached corpus (REF_EQ_IDS):
  1. round-trip (two independent decoders) on equation-domain samples
...
samples = [
    ...
    "Operators present: { (x2), * (x1). Query operator * shares examples: 31*15.",
    "absolute difference [reversed operands, reversed result]: 96,25 -> |96-25|=71 -> 17",
    "Result is negative; the operator symbol replaces the sign: -03 -> 03!",
    CHAT_PRE + "x" + PROMPT_SUFFIX + CHAT_POST,
    "The answer in \\boxed{–} is \\boxed{631}\n</think>\n\\boxed{631}<|im_end|>",
]
rt_ok = True
...
# ---------- Check 2: reference-match on existing deduce entries ----------
prompts = {r["id"]: r["prompt"] for r in csv.DictReader(open(TRAIN_CSV, newline=""))}
ref_ok = True
boundary_info = []
for pid in REF_EQ_IDS:
...
report("2. reference-match (ids == original, prompt+completion)", ref_ok,
       f"{len(REF_EQ_IDS)} equation_numeric_deduce entries")
