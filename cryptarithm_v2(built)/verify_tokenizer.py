"""MANDATORY GATE for cryptarithm_v2: prove tokenizer fidelity before any generation.

4 checks referenced against 3 EXISTING cryptarithm_deduce entries (REF_CRYPT_IDS):
  1. round-trip (two independent decoders) on cryptarithm-domain samples
...
samples = [
    ...
    "Operator '*' examples: %|*\"| = %|\"|. concatenation matches.",
    "Symbol->digit: ` =1, ! =2, [ =3. Apply: |[*([ -> |[([",
...
# ---------- Check 2: reference-match on existing cryptarithm entries ----------
prompts = {r["id"]: r["prompt"] for r in csv.DictReader(open(TRAIN_CSV, newline=""))}
ref_ok = True
boundary_info = []
for pid in REF_CRYPT_IDS:
...
report("2. reference-match (ids == original, prompt+completion)", ref_ok,
       f"{len(REF_CRYPT_IDS)} cryptarithm_deduce entries")
