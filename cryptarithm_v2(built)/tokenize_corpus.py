"""Tokenize cryptarithm_v2 into the exact corpus segment format, enforcing conciseness caps.

Same mechanics as bit_v2/equation_v2: masked prompt (chat template) + trained completion
`{reasoning}\n</think>\n\boxed{answer}<|im_end|>`, interleaved segments, byte-exact
round-trip with two independent decoders per entry.

HARD conciseness enforcement on COMPLETION (trained) tokens: every trace <= 4500;
distribution must satisfy median < 3500 and p90 < 3500 (asserted; build FAILS otherwise).
...
def main() -> None:
    ...
    print("(base model on this family: ~7,610 gen tokens, 98.8% truncated -- what we fix)")
